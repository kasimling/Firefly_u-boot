...
int connector_pclist_parse_dt(struct connector_state *conn_state, const void *blob, int panel)
{
	struct rockchip_pwrctr *pwrctr = &(conn_state->pwrctr);
	struct list_head *pclist_head = &(pwrctr->pclist_head); 
	int root = fdt_subnode_offset(blob, panel, "power_ctr");
	int child;
	...
	if (root <= 0) {
		printf("can't find power_ctr node for panel\n");
		return 0;
	}
	pwrctr->debug = fdtdec_get_int(blob, root,"rockchip,debug", 0);

	for (child = fdt_first_subnode(blob, root);
	     child >= 0;
	     child = fdt_next_subnode(blob, child)) {
		pclist = malloc(sizeof(*pclist));
		if (!pclist) {
		...
		if (len < 32)
			strcpy(pclist->pc.name, name);
		val = fdtdec_get_int(blob, child, "rockchip,power_type", -1);

		fdtdec_decode_gpio(blob, child, "gpios",
				&pclist->pc.gpio);
		pclist->pc.atv_val =  !(pclist->pc.gpio.flags &
				OF_GPIO_ACTIVE_LOW);
		gpio_direction_output(pclist->pc.gpio.gpio, !pclist->pc.atv_val);

		pclist->pc.delay = fdtdec_get_int(blob, child,
							"rockchip,delay", 0);
	...
	if(pwrctr->debug > 0) {
		list_for_each(pos, pclist_head) {
			pclist = list_entry(pos, struct rockchip_pwrctr_list,
					list);
			printf("pwrctr_name:%s, parse gpio:%d, atv_val=%d,delay:%dms\n",
					pclist->pc.name, pclist->pc.gpio.gpio, 
...
void connector_panel_power_unprepare(struct display_state *state)
{
	...
	const struct rockchip_panel *panel = panel_state->panel;
	struct connector_state *conn_state = &state->conn_state;
	struct list_head *pclist_head = &(conn_state->pwrctr.pclist_head); 
	struct list_head *pos;
	struct rockchip_pwrctr_list *pclist;
	struct pwrctr *pc;
