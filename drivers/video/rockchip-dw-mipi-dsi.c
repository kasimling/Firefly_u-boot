...
#define FDT_GET_INT(val, name) \
	...
	DBG("dsi->lanes=%d, dsi->format=%#x, dsi->mode_flags=%#x, dsi->channel=%d\n", 
			dsi->lanes,dsi->format,dsi->mode_flags, dsi->channel);
	return 0;
}
...
static void rockchip_dw_mipi_dsi_read_allregs(struct dw_mipi_dsi *dsi)
{
	int i;
	u32 val;
